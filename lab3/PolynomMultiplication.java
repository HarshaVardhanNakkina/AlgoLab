...
public class PolynomMultiplication {
  ...
  static void printArray(ComplexNumber[] arr) {
    for(ComplexNumber el: arr) System.out.print(el.real + " ");
    System.out.println();
  }

  static int getExtendedSize(int deg) {

    int n = (int) (Math.log10(deg - 1) / Math.log10(2));
    int extendedSize = (int) Math.pow(2, n + 1) + 1;
  ...
  static ComplexNumber[] TransformPolynomial(ComplexNumber[] poly, int size, int inversion) {
    ...
    for(int i = 0; i < size / 2; i++) {
      ComplexNumber tmp1 = Y0[i].add(Y1[i].mul(wn));

      Y[i] = new ComplexNumber();
      Y[i].real = tmp1.real;
      Y[i].img = tmp1.img;

      ComplexNumber tmp2 = Y0[i].sub(Y1[i].mul(wn));
  ...
  static void multiplyPolynomials(ComplexNumber[] a, ComplexNumber[] b, ComplexNumber[] c, int size) {
  ...
  public static void main(String[] args) throws IOException{
    ...
    int extendedSize = getExtendedSize(Math.max(degA, degB));
